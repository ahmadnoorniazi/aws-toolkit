...
export default function MaterialTableDemo() {
  ...
  const addRow = async (newData) => {
    setIsLoading(true);
    const called = await addRecord('product/create',newData)
    getProductData()
      setIsLoading(false)
  }
  ...
  const updateRow = async (newData) => {
    setIsLoading(true);
    const called = await updateRecord('product/update',newData._id,newData)
    getProductData()
    setIsLoading(false)
  }
  ...
  const deleteRow = async (newData) => {
    setIsLoading(true);
    const called = await deleteRecord('product/remove',newData._id)
    getProductData()
    setIsLoading(false)
  }
