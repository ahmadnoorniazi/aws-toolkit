...
export default function MaterialTableDemo() {
    ...
    columns: [
      {
        ...
        field: "product_name",
        editComponent: data => <Products data={data} setSelProd={setSelProd} />
      },
  ...
  const filterBill = async (startDate, endDate) => {
    setIsLoading(true);
    try {
      const called = await getRecord(
        `received_stock/filter/${startDate}/${endDate}`
      );
      setData(called.data ? called.data : []);
      setIsLoading(false);
    } catch (error) {
  ...
  const addRow = async newData => {
    ...
    if (sel_pro) {
      ...
      setIsLoading(true);
      try {
        await addRecord("received_stock/create", addData);
        getData();
        setIsLoading(false);
      } catch (error) {
        setIsLoading(false);
  ...
  const deleteRow = async newData => {
    setIsLoading(true);
    try {
      await deleteRecord("received_stock/remove", newData._id);
      getData();
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
  ...
  const updateRow = async (newData, oldData) => {
    ...
    if (sel_pro && Object.keys(sel_pro).length > 0) {
      ...
      const addData = {
        ...
        product_name: name,
        product_price: actual_price,
        product_quantity: validQuan,
        total_bill_amount: total
      };
      setIsLoading(true);
      try {
        await updateRecord("received_stock/update", newData._id, addData);
        getData();
        setIsLoading(false);
        setSelProd(null);
      } catch (error) {
      ...
      setIsLoading(true);
      try {
        await updateRecord("received_stock/update", newData._id, {
          ...newData,
          product_quantity: Number(getQuan),
          total_bill_amount: total
        });
        getData();
        setIsLoading(false);
  ...
  return (
    <Fragment>
      <DatePicker
        getDateValues={filterBill}
        resetTable={() => setReset(!reset)}
      />
      <Table
        isLoading={isLoadig}
