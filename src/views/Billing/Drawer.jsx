...
export default function FormDialog({isDisable}) {
  ...
  const [disable, setIsDisable ] = React.useState(true);
  const {cart,setCart, total, discount, setDiscount, customerName,setCustomerName,taxName,setTaxName,taxValue,setTaxValue} = useContext(CartState)
  ...
  const saveBill = async (isPaid) => {
    ...
    const called = await addRecord('billing/create',data)
    }
      ...
      <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
        ...
        <DialogContent>
         ...
         OR select from current customers
          ...
          renderInput={params => <TextField  {...params} label="Select Customer name" variant="outlined" />}
        />
        <DialogContentText style={{marginTop: "20px"}}>
        Discount
      </DialogContentText>
